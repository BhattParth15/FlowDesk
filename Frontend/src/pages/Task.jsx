...
function Task() {
    ...
    }, [page, limit, search, statusFilter, assigneeFilter, hasPermission, selectedProject]);
    ...
    const fetchTasks = async () => {
        try {
            ...
            if (selectedProject && selectedProject._id !== "all") {
                url += `/task?projectId=${selectedProject._id}&page=${page}&limit=${limit}&search=${search}&status=${statusFilter}&assignedTo=${assigneeFilter}&type=task`;
            }// else {
            //     url += `/task?page=${page}&limit=${limit}&search=${search}&status=${statusFilter}&assignedTo=${assigneeFilter}&type=task`
            // }
            else if (selectedProject?._id === "all" && selectedProject.projectIds?.length) {
            url += `/task?page=${page}&limit=${limit}&search=${search}&status=${statusFilter}&assignedTo=${assigneeFilter}&type=task&projectIds=${selectedProject.projectIds.join(",")}`;
        }
    ...
    const handleAssigneeChange = async (task, userId) => {
        try {
            await axios.put(`${API_URL}/task/${task._id}`,
                {
                    name: task.name,
    ...
    const handleBulkData = () => {
        if (!selectedProject || selectedProject._id === "all") {
            setError("Please select a Project before uploading bulk Task.");
            setTimeout(() => setError(""), 3000);
            return;
        }
        navigate("/bulk/upload?type=task")
    }

    const handleEditClick = (item) => {
        if (!selectedProject || selectedProject._id === "all") {
            setError(`Please select a Project before editing this Task.`);
            setTimeout(() => setError(""), 3000);
            return;
        }
        navigate(`/task/edit/${item._id}`, { state: item })
    }
    ...
    const column = [
        ...
        //         ) : (
        //           <div
        //             className="flex justify-center gap-2 pl-0"
        //             onClick={() => setEditingTaskId(item._id)}
        //           >
        //             {name !== "Unassigned" && (
        //               <div className="w-8 h-8 flex items-center justify-center rounded-full bg-blue-400 text-indigo-700 text-xs font-semibold">
        //                 {initials}
        //               </div>
        //             )}
